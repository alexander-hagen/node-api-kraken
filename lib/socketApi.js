...
//          default: // logger.info('ws ping pending. readyState='+this._ws.readyState);
//            break;
//        };
...
KrakenSocket.prototype.addOrder = async function(opts) { // https://docs.kraken.com/api/docs/websocket-v2/add_order/
  ...
  return await new Promise(async (resolve, reject) => {
    ...
    if(output.success) { resolve(output.result); } 
    else { reject(output.error); };
  });
};

KrakenSocket.prototype.amendOrder = async function(opts) { // https://docs.kraken.com/api/docs/websocket-v2/amend_order/
  const key=(++this.socket._id);
  const params=Object.assign({"token": this.socket.auth.token},opts);
  const options={
    "method":"amend_order",
...
KrakenSocket.prototype.editOrder = async function(opts) { // https://docs.kraken.com/api/docs/websocket-v2/edit_order/
  const key=(++this.socket._id);
  const params=Object.assign({"token": this.socket.auth.token},opts);
  const options={
    "method":"edit_order",
...
KrakenSocket.prototype.cancelOrder = async function(opts) { // https://docs.kraken.com/api/docs/websocket-v2/cancel_order/
  const key=(++this.socket._id);
  const params=Object.assign({"token": this.socket.auth.token},opts);
  const options={
    "method":"cancel_order",
...
KrakenSocket.prototype.cancelAllOrders = async function(opts) { // https://docs.kraken.com/api/docs/websocket-v2/cancel_all/
  const key=(++this.socket._id);
  const params=Object.assign({"token": this.socket.auth.token},opts);
  const options={
    "method":"cancel_all",
...
KrakenSocket.prototype.cancelAllOrdersAfter = async function(opts) { // https://docs.kraken.com/api/docs/websocket-v2/cancel_after/
  const key=(++this.socket._id);
  const params=Object.assign({"token": this.socket.auth.token},opts);
  const options={
...
KrakenSocket.prototype.addOrderBatch = async function(opts) { // https://docs.kraken.com/api/docs/websocket-v2/batch_add/
  const key=(++this.socket._id);
  const params=Object.assign({"token": this.socket.auth.token},opts);
  const options={
    "method":"batch_add",
...
KrakenSocket.prototype.cancelOrderBatch = async function(opts) { // https://docs.kraken.com/api/docs/websocket-v2/batch_cancel/
  const key=(++this.socket._id);
  const params=Object.assign({"token": this.socket.auth.token},opts);
  const options={
    "method":"batch_cancel",
